...
try:
    #加载一个已经训练好的我的model
    os.path.exists('my_model')
    sequential = tf.keras.models.load_model('my_model')
    sequential.trainable = False
    inputslayer = tf.keras.Input(shape=(192,192,3))
    sequential = sequential(inputslayer)
    model = tf.keras.Model(inputslayer, sequential,name= "load_model")
    ...
    if os.path.exists(checkpoint_path):
      ...
      model.fit(ds, epochs=3, steps_per_epoch=steps_per_epoch, callbacks = [cp_callback],validation_split=0.0) #validation_split=0.0 为默认，可以修改
    """保存整个模型
